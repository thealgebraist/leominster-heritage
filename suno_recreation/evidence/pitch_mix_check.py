...
for tr in TRACKS:
 ...
 for label,z in [('mix',y),('harmonic',h)]:
  ...
  np.savetxt(r/'pitch'/(tr['id']+'_'+label+'.csv'),np.column_stack([ts,f0,prob,keep]),delimiter=',',header='seconds,estimated_hz,voiced_probability,retained',comments='')
 (r/'pitch'/(tr['id']+'_mix_check.json')).write_text(json.dumps({'method':'librosa pYIN, 8 kHz source mixture and HPSS harmonic component; f0 70-650 Hz; probability >=.8; RMS above 30th percentile/floor .003','warning':'Includes backing, which can dominate detected pitch; quantiles are candidate periodicities, not an established sung line.'},indent=2))
