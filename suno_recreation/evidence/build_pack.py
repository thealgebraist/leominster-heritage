...
for t in TRACKS:
 asset=asset_stem(t['id'])
 p=ROOT/'tracks'/t['slug'];p.mkdir(parents=True,exist_ok=True)
 m=MUSIC[t['id']]
 signal=json.loads((ROOT/'evidence/signal'/(asset+'.json')).read_text())
 ...
 descs=[json.loads(x.read_text()) for x in sorted((ROOT/'evidence/audio_descriptions').glob(asset+'_*.json'))]
...
'''
 stem=asset
 if (ROOT/'evidence/second_pass'/(stem+'_backing.json')).exists():body+=f"- [Separated-backing interpretation](../../evidence/second_pass/{stem}_backing.json)\n"
 ocr=ROOT/'evidence/ocr_results'/(asset+'.json')
 frames=ROOT/'evidence/frames'/(t['id']+'.jpg')
 if ocr.exists():body+=f"- [Sampled-frame OCR results (automated and noisy)](../../evidence/ocr_results/{asset}.json)\n"
 if frames.exists():body+=f"- [Sampled source-frame contact sheet](../../evidence/frames/{t['id']}.jpg)\n"
 if (ROOT/'evidence/second_pass'/(asset+'_vocals.json')).exists():body+=f"- [Targeted vocal interpretation](../../evidence/second_pass/{asset}_vocals.json)\n"
 pitch=json.loads((ROOT/'evidence/pitch'/(asset+'.json')).read_text())
 body+=f"- [Pitch tracking diagnostic](../../evidence/pitch/{asset}.json): {pitch['retained_fraction']:.1%} of frames passed the strict voicing filter; too sparse to establish a complete melody.\n"
 vosk=ROOT/'evidence/asr_vosk'/(asset+'.json')
 if vosk.exists():body+=f"- [Independent small-model Vosk ASR output (raw; noisy)](../../evidence/asr_vosk/{asset}.json)\n"
 large_vosk=ROOT/'evidence/asr_vosk_large'/(asset+'.json')
 if large_vosk.exists():body+=f"- [Higher-capacity Vosk ASR output (raw; noisy)](../../evidence/asr_vosk_large/{asset}.json)\n"
 mix_pitch=ROOT/'evidence/pitch'/(asset+'_mix_check.json')
 if mix_pitch.exists():body+=f"- [Original-mix and harmonic pitch tracking diagnostic](../../evidence/pitch/{asset}_mix_check.json)\n"
 for d in descs:
  evidence_name=asset+"_"+f"{int(d['start_s']):02d}"+".json"
...
</style><main><div class="meta">9 CLIPS · SOURCE LYRICS + PRONUNCIATION + SUNO PROMPTS</div><h1>Architectural music</h1><p>Recreation recipes for all nine source videos. Use each transcript to inspect the evidence and the separate prompt boxes to generate an approximation.</p><p class="flag">Audio descriptions and pronunciation details are model-assisted estimates, not a verified score. Exact repeated-word counts and a few phrases remain unresolved. Suno output has not been generated or tested. <a href="README.md">Read the evidence limits.</a></p><nav>''']
