...
The door-knocker source is `source_videos/door-knockers.mp4`. Its transcript combines Whisper large-v3-turbo output, a noisy independent Vosk pass, and sampled caption OCR. The captions support the named phrases noted in the track report; ambiguous wording is labeled there.
...
For Columns, Doors, and Windows, `rolling_tonality.py` repeats the comparison in 4-second windows with a 2-second hop. This sensitivity check shows whether an apparent tonal change depends on the original 8-second cut. It places the Doors transition around 14–18 seconds with a C-minor-like middle and F-minor-like return near 28–32 seconds; the Windows G-minor profile predominates with brief G-major-like windows; Columns repeatedly alternates E-flat and B-flat major profile labels. The overlapping measurements are still fixed audio windows and do not establish musical bars, chord function, or a score.
...
## Door-knocker analysis and reference audit
