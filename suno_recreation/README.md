...
- **Tonality:** whole-clip and 8-second section-level pitch-class profiles provide candidates only. The section pass compares the original mix with an estimated backing stem; where those agree across repeated sections, the report describes a tonal-center or mode hypothesis for testing, not an established key or chord progression. Voice, percussion overtones, short duration, sampled speech, and separation artifacts can bias the profiles.
